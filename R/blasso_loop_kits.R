...
## ...
## JTN LISÄÄ? Esim 0/1 halutaanko CPU time yms statistiikkan tulostusehtoja?
##
...
blasso_loop_kits <- function(X,Y,K,C,iprint=1,start=1,...){
  ...
  if(!is.double(C)) { storage.mode(C) <- 'double' }	
    ...
    result <- .Call(c_coxdbdc_loop_kits_f,as.integer(nft),as.integer(nrecord),as.integer(nkits),as.double(inX),c(as.double(Y[,1]),as.double(Y[,2])),
                    as.double(as.vector(C)),as.double(inK),iprint,start)
  }
  if(exists(result)){
    return(result)
  }else{

...
The CASS-DBDC algorithm is used to solve the problem $\min_{\boldsymbol{\beta}} f(\bbeta)$. The algorithm combines the penalty function approach with the double bundle method (DBDC) VIITE!. As starting point usually influences the solution in nonconvex problems, such as our problem, an incremental approach is used to select the set of starting points. In particular, we start to solve the problem with one kit allowed and then increase the number until maximum. The solution with $i$ kits is then used to derive new starting points to the problem with $i+1$ kits. MAINITAANKO MYÖS ylhäältä alas?

The code cannot (yet?) handle sparse input-matrixes. We advise the user to see section "Data" in this vignette to gain knowledge about what kind of input data is required. KANNATTAAKO SANOA?

The package produces a S4 object (casso object) which includes, either `coxph` or `glm` object with coefficients obtained by the code. These models have their own predict functions and thus can be used as such. The `survival` package is required to predict with `coxph` and the `stats` package is required to predict with `glm`.  See … VIITTEITÄ!.
...
The package also includes methods for plotting and a function for cross validation (toimiiko cv.casso?).
...
- **Response matrix**: Input matrix `y` should not have missing values. Row number should be equal to row number of matrix `x`. In the case of survial prediction with Cox, there should be two columns with time ($\geq0$ ONKO AIDOSTI SUUREMPI?) in the first column and event (0/1) in the secont. In the cases of mse and logistic, only one column is needed (VAI ONKO VEKTORI?). Additionally, in the case of logistic, the response should be binomial (0/1).
...
- **Cost vector**: Input vector `w` should have length equal to the row number of kit matrix `k`. MITES PUUTTUVAT?




## Cox
Sepustusta Cox-mallista? Kuten glmnet vignette?
...
# tämmönen olis kiva, ei tartte miettiä onko $ vai []:
#kperk(tmp,10)
#bperk(tmp,10)
```

In addition to target function values, the casso object "tmp.cox" includes also calculation for goodness of fit. In the case Cox this is concordance index (VIITE?). Let's plot goodness of fit with target function value:
```{r}
...
Tähän lätinää mean square errorista ja lin regr modelista?
...
In addition to target function values, the `casso` object "tmp.mse" includes also calculation for goodness of fit. In the case of mse the measure is the mean square error itself (VIITE mse:hen?). Let's plot goodness of fit with target function value:
```{r}
...
## Logistic
Tähän lätinää logisticista ja glm modelista?
